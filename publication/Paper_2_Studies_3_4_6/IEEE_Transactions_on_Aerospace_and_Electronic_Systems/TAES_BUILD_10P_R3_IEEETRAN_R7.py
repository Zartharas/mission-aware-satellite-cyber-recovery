#!/usr/bin/env python3
"""Revision-7 typography builder for the balanced eight-page TAES Paper-2 candidate.

R5 and R6 stopped before compilation because their guards incorrectly treated
the document-wide ``\\raggedright`` count as the localization count. The base
generated IEEEtran TeX already contains unrelated ``\\raggedright`` commands.

R7 preserves the exact R5/R6 typography transformation: start from the validated
R2 breakable-identifier TeX and wrap only the unique first Study-6
state-definition paragraph in ``\\begingroup\\raggedright ... \\par\\endgroup``.
The guard now verifies the correct structural invariant: exactly one additional
``\\raggedright`` is introduced and exactly one Study-6 wrapper marker exists.

No visible manuscript character, canonical Markdown prose, frozen result, table
value, reference, supplement, geometry setting, or publisher-facing R9 artifact
is changed. ``\\sloppy`` and forced ``\\linebreak`` hints are forbidden.
"""
...
OUT_TEX = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R7_DEV.tex"
OUT_PDF = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R7_DEV.pdf"
OUT_LOG = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R7_DEV.log"
OUT_AUDIT = ROOT / "TAES_10P_R3_IEEETRAN_R7_BUILD_AUDIT.txt"

_LOCALIZED_RAGGEDRIGHT_BLOCKS = 0
_PREEXISTING_RAGGEDRIGHT_COUNT = 0
_FINAL_RAGGEDRIGHT_COUNT = 0

_STUDY6_WRAPPER_PREFIX = "\\begingroup\\raggedright\nStudy 6 ("


def _localize_study6_first_paragraph(tex: str) -> str:
    global _LOCALIZED_RAGGEDRIGHT_BLOCKS
    parts = tex.split("\n\n")
    hits = [
    ...
    if len(hits) != 1:
        raise SystemExit(
            "ERROR: R7 expected exactly one first Study-6 state-definition paragraph; "
            f"found {len(hits)}"
        )
    idx = hits[0]
    paragraph = parts[idx]
    if r"\raggedright" in paragraph or r"\sloppy" in paragraph:
        raise SystemExit("ERROR: R7 target paragraph already contains layout relaxation")
    parts[idx] = "\\begingroup\\raggedright\n" + paragraph + "\\par\\endgroup"
    _LOCALIZED_RAGGEDRIGHT_BLOCKS = 1
    return "\n\n".join(parts)
...
def make_tex_r7(md: str) -> str:
    ...
    if r"\sloppy" in tex:
        raise SystemExit("ERROR: R7 unexpectedly contains \\sloppy")
    if re.search(r"\\linebreak\[[24]\]", tex):
        raise SystemExit("ERROR: R7 unexpectedly contains a forced linebreak hint")

    if _FINAL_RAGGEDRIGHT_COUNT != _PREEXISTING_RAGGEDRIGHT_COUNT + 1:
        raise SystemExit(
            "ERROR: R7 localized raggedright delta is not exactly one: "
    ...
    if wrapper_hits != 1:
        raise SystemExit(
            "ERROR: R7 expected exactly one structural Study-6 raggedright wrapper; "
            f"found {wrapper_hits}"
        )

    if _LOCALIZED_RAGGEDRIGHT_BLOCKS != 1:
        raise SystemExit(
            "ERROR: R7 localization block counter is not one: "
            f"{_LOCALIZED_RAGGEDRIGHT_BLOCKS}"
        )
...
def write_audit(result: dict[str, str | int | float | bool]) -> None:
    ...
    lines = [
        "TAES_10P_R3_IEEETRAN_R7_BUILD_AUDIT",
        ...
        f"localized_raggedright_blocks={_LOCALIZED_RAGGEDRIGHT_BLOCKS}",
        "localized_raggedright_scope=FIRST_STUDY6_STATE_DEFINITION_PARAGRAPH_ONLY",
        "r7_guard=STRUCTURAL_WRAPPER_AND_PLUS_ONE_RAGGEDRIGHT_DELTA",
...
def print_overfull_context() -> None:
    ...
    print("TAES_10P_R3_R7_OVERFULL_CONTEXT_BEGIN")
    if not matches:
        print("NONE")
    else:
    ...
    print("TAES_10P_R3_R7_OVERFULL_CONTEXT_END")
...
def main() -> None:
    ...
    core.r1.base.make_tex = make_tex_r7
    ...
    print("TAES_10P_R3_IEEETRAN_BUILD_REVISION=7")
    print("r7_change=R6_GLOBAL_COUNT_GUARD_REPLACED_BY_STRUCTURAL_LOCALIZATION_GUARD")
    ...
    print(f"localized_raggedright_blocks={_LOCALIZED_RAGGEDRIGHT_BLOCKS}")
    print("localized_raggedright_scope=FIRST_STUDY6_STATE_DEFINITION_PARAGRAPH_ONLY")
    print("r7_guard=STRUCTURAL_WRAPPER_AND_PLUS_ONE_RAGGEDRIGHT_DELTA")
