...
echo "TAES_SUPPLEMENT_R7_FINAL_DEV_GATE=START"
echo "branch=$(git -C "$REPO_ROOT" branch --show-current)"
echo "head=$(git -C "$REPO_ROOT" rev-parse HEAD)"
echo "pandoc=$(pandoc --version | head -1)"
echo "latexmk=$(latexmk -v | head -1)"

python3 "$ROOT/TAES_BUILD_10P_SUPPLEMENT_R7.py"
rc=$?
if [[ "$rc" -ne 0 ]]; then
  echo "TAES_SUPPLEMENT_R7_FINAL_DEV_GATE=FAIL_BUILD"
  exit "$rc"
fi

PDF="$ROOT/TAES_10P_R3_SUPPLEMENTARY_MATERIAL_R7_DEV.pdf"
TEX="$ROOT/TAES_10P_R3_SUPPLEMENTARY_MATERIAL_R7_DEV.tex"
LOG="$ROOT/TAES_10P_R3_SUPPLEMENTARY_MATERIAL_R7_DEV.log"
AUDIT="$ROOT/TAES_10P_R3_SUPPLEMENTARY_R7_BUILD_AUDIT.txt"
...
pdftotext "$PDF" - | grep -E 'Supplementary Figure S1|Appendix A|Appendix B|Appendix C|Appendix D|Appendix E|Table S1|Table S2|Table S3' | head -80 || true
...
echo "TAES_SUPPLEMENT_R7_FINAL_DEV_GATE=PASS"
exit 0
