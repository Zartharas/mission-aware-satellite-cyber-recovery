#!/usr/bin/env python3
"""Revision-5 typography builder for the balanced eight-page TAES Paper-2 candidate.

R4 retained two small horizontal overflows in the same first Study-6 paragraph
after targeted forced-break experiments. R5 deliberately returns to the cleaner
R2 generated-LaTeX state, preserving its breakable monospaced identifiers, and
wraps exactly that one Study-6 state-definition paragraph in a localized
``\\raggedright`` group.
...
is changed. The rest of the manuscript remains fully justified. ``\\sloppy`` is
forbidden.
"""
...
OUT_TEX = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R5_DEV.tex"
OUT_PDF = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R5_DEV.pdf"
OUT_LOG = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R5_DEV.log"
OUT_AUDIT = ROOT / "TAES_10P_R3_IEEETRAN_R5_BUILD_AUDIT.txt"
...
def _localize_study6_first_paragraph(tex: str) -> str:
    ...
    if len(hits) != 1:
        raise SystemExit(
            "ERROR: R5 expected exactly one first Study-6 state-definition paragraph; "
            f"found {len(hits)}"
        )
    idx = hits[0]
    paragraph = parts[idx]
    if r"\raggedright" in paragraph or r"\sloppy" in paragraph:
        raise SystemExit("ERROR: R5 target paragraph already contains layout relaxation")
...
def make_tex_r5(md: str) -> str:
    ...
    if r"\sloppy" in tex:
        raise SystemExit("ERROR: R5 unexpectedly contains \\sloppy")
    if tex.count(r"\raggedright") != 1:
        raise SystemExit(
            f"ERROR: R5 expected one localized \\raggedright; found {tex.count(r'\\raggedright')}"
        )
    return tex
...
def write_audit(result: dict[str, str | int | float | bool]) -> None:
    ...
    lines = [
        "TAES_10P_R3_IEEETRAN_R5_BUILD_AUDIT",
...
def print_overfull_context() -> None:
    ...
    print("TAES_10P_R3_R5_OVERFULL_CONTEXT_BEGIN")
    if not matches:
        print("NONE")
    else:
    ...
    print("TAES_10P_R3_R5_OVERFULL_CONTEXT_END")


def main() -> None:
    core.verify_r3_binding()

    # Rebind only development outputs. The exact balanced R3 Markdown source and
    # frozen R9 fallback binding remain those established by the R1 short builder.
    ...
    core.r1.base.make_tex = make_tex_r5
    ...
    print("TAES_10P_R3_IEEETRAN_BUILD_REVISION=5")
    print("r5_change=GENERATED_LATEX_LOCALIZED_STUDY6_RAGGEDRIGHT_ONLY")
