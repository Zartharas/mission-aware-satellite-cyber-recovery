#!/usr/bin/env python3
"""Revision-6 typography builder for the balanced eight-page TAES Paper-2 candidate.

R5 stopped before compilation because its guard searched for two literal
backslashes when verifying the single LaTeX ``\\raggedright`` command that it
had inserted. R6 preserves the exact R5 typography strategy and changes only
that Python verification logic.

The exact balanced-R3 Markdown source, supplement, frozen science, tables,
references, geometry, and publisher-facing R9 fallback remain unchanged.
``\\sloppy`` and forced ``\\linebreak`` hints are forbidden.
"""
...
OUT_TEX = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R6_DEV.tex"
OUT_PDF = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R6_DEV.pdf"
OUT_LOG = ROOT / "TAES_10P_R3_MANUSCRIPT_IEEETRAN_R6_DEV.log"
OUT_AUDIT = ROOT / "TAES_10P_R3_IEEETRAN_R6_BUILD_AUDIT.txt"
...
def _localize_study6_first_paragraph(tex: str) -> str:
    ...
    if len(hits) != 1:
        raise SystemExit(
            "ERROR: R6 expected exactly one first Study-6 state-definition paragraph; "
            f"found {len(hits)}"
        )
    idx = hits[0]
    paragraph = parts[idx]
    if r"\raggedright" in paragraph or r"\sloppy" in paragraph:
        raise SystemExit("ERROR: R6 target paragraph already contains layout relaxation")
...
def make_tex_r6(md: str) -> str:
    ...
    if r"\sloppy" in tex:
        raise SystemExit("ERROR: R6 unexpectedly contains \\sloppy")
    ragged_count = tex.count(r"\raggedright")
    if ragged_count != 1:
        raise SystemExit(
            f"ERROR: R6 expected one localized \\raggedright; found {ragged_count}"
        )
    if re.search(r"\\linebreak\[[24]\]", tex):
        raise SystemExit("ERROR: R6 unexpectedly contains a forced linebreak hint")
    return tex
...
def write_audit(result: dict[str, str | int | float | bool]) -> None:
    ...
    lines = [
        "TAES_10P_R3_IEEETRAN_R6_BUILD_AUDIT",
        ...
        "r6_guard_fix=SINGLE_BACKSLASH_LATEX_COMMAND_MATCHING_ONLY",
...
def print_overfull_context() -> None:
    ...
    print("TAES_10P_R3_R6_OVERFULL_CONTEXT_BEGIN")
    if not matches:
        print("NONE")
    else:
    ...
    print("TAES_10P_R3_R6_OVERFULL_CONTEXT_END")
...
def main() -> None:
    ...
    core.r1.base.make_tex = make_tex_r6
    ...
    print("TAES_10P_R3_IEEETRAN_BUILD_REVISION=6")
    print("r6_change=R5_SINGLE_BACKSLASH_GUARD_FIX_ONLY")
    ...
    print("r6_guard_fix=SINGLE_BACKSLASH_LATEX_COMMAND_MATCHING_ONLY")
