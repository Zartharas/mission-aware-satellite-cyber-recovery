...
The prior sufficient bound used `ceil(8B/d_min)`. When `8B/d_min` was an integer and the shortest effective future window ended at the horizon, that rate could deliver exactly `B` modeled bytes exactly at the horizon, which violates the strict-before-horizon success rule. The corrected `floor(8B/d_min) + 1` construction makes the sufficient bound strictly greater in those exact-divisibility cases.
