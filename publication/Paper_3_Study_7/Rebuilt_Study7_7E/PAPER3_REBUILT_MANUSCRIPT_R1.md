...
**Paper 3 — venue-neutral rebuilt manuscript R1**  
**Studies:** Study 7 / S7-LSO-001 and Study 7E / S7E-AERC-001  
**Draft state:** VENUE_NEUTRAL_R1__FROZEN_SCIENCE_ONLY__SCIENTIFIC_AUDIT_REQUIRED  
**Submission authorization:** NOT GRANTED

## Abstract

Satellite cyber-recovery logic must decide whether to enter a recovery state from evidence that may itself be incomplete, stale, corrupted, or produced by a compromised trust path. This paper studies that problem in two separately frozen finite experiments. Study 7 isolates the information-sufficiency boundary of learned recovery selection. Across 1,033 modeled observations, a visible-only learned selector exactly reproduced its deterministic visible decision boundary but could not distinguish safe and signed-but-false states whose policy-visible inputs were identical. A corroboration-aware learner avoided the unsafe decision when the added observable disagreed with the compromised path, but the failure returned when corroboration was falsely positive in correlation with that path. Study 7E prospectively extends the question into a cFS-grounded reference architecture with explicit source, key, execution, transport, and authorization trust domains; deterministic and learned policy pairs receiving byte-identical inputs; frozen decision-tree models; thirteen fault profiles; and held-out evaluation on unseen faults and higher-separation topologies. Its 196 held-out scenarios produced 784 policy decisions with zero invalid scenarios and zero audit mismatches. Equal-information deterministic/learned pairs disagreed in 67 of 196 base-information scenarios and 72 of 196 corroborated scenarios. Aggregate corroboration reduced unsafe proceeds for the deterministic policy from 9 to 4 while increasing false-conservative holds from 31 to 45, whereas the learned corroborated policy increased unsafe proceeds from 21 to 31 while reducing false-conservative holds from 26 to 12. Topology-controlled results were also non-monotonic: deterministic corroboration removed one unsafe proceed only under full separation, while learned corroboration showed adverse transfer as separation increased. The combined evidence supports an assurance conclusion rather than a policy ranking: recovery correctness depends on both information availability and the architecture that produces and shares that information, and neither corroboration nor greater trust-domain separation is universally beneficial at the policy-output level.

**Keywords:** satellite cybersecurity; cyber recovery; machine-learning assurance; core Flight System; trust domains; corroboration; partial observability; common-cause failure
...
### 2.3 Spacecraft recovery architecture
...
Both learned policies use deterministic empirical-risk minimization over a bounded integer linear-threshold hypothesis class. The frozen L0 parameters are quality weight 2, authorization weight 1, security weight -1, and threshold 12. The frozen L1 parameters are quality weight 2, authorization weight 0, corroboration weight 1, security weight -1, and threshold 12. Both have zero training errors.
...
Both studies are deterministic finite research experiments. Their completeness applies to the defined modeled populations, not to all spacecraft cyber-recovery conditions.
...
[7] T. Le, B. Juba, and R. Stern, "Learning Safe Action Models with Partial Observability," *Proceedings of the AAAI Conference on Artificial Intelligence*, 2024. Available: https://ojs.aaai.org/index.php/AAAI/article/view/29995

[8] Y. Jing and Y. Nakahira, "Safety Certificate against Latent Variables with Partially Unidentifiable Dynamics," *Proceedings of the 42nd International Conference on Machine Learning*, 2025. Available: https://proceedings.mlr.press/v267/jing25b.html
...
[10] NASA Goddard Space Flight Center, "Core Flight Executive (cFE)," NASA Software Catalog / NASA cFE repository. Available: https://github.com/nasa/cFE
...
[13] NASA, "Operational Simulator for Space Systems (NOS3)," repository and Small Spacecraft Systems Virtual Institute mission-design tools. Available: https://github.com/nasa/nos3 and https://www.nasa.gov/smallsat-institute/space-mission-design-tools/
