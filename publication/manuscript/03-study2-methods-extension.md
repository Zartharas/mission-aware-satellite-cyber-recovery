...
### 3.12.8 Pre-runtime security assurance and bounded formal model checking

Before frozen Study-2 campaign execution, the assurance pipeline combined implementation-bound conformance/security testing with bounded TLA+ model checking of selected trust and recovery properties. Three specifications were checked with SANY and TLC. `Study1P7` covered Study-1 selector conformance and oracle isolation; its configuration checked `TypeOK`, `OracleIsolation`, `EvidenceAssessmentConformance`, and `P7DecisionConformance` and explored 48 distinct states. `TrustedRecovery` checked type correctness, oracle isolation, ground-truth immutability, evidence-sufficient and evidence-fallback branch integrity, and trusted-recovery soundness over 385 distinct states. `AdversarialEvidence` extended the model to treatment immutability, verifier and independent-trust-anchor boundaries, evidence qualification, known-invalid evidence rejection, contradiction handling, and trusted-recovery soundness; the frozen pre-runtime provenance records 540 generated states, 400 distinct states, and search depth 3.

The same pre-runtime assurance record also reports 41 deterministic security tests, eight property-based security tests, and a seven-of-seven semantic mutation assay, together with explicit exclusion of the research adjudication oracle from runtime access. These checks were completed before the empirical campaign and therefore served as design/implementation assurance rather than post-hoc explanation of the observed Study-2 results.

This formal layer is deliberately bounded. The TLA+ specifications abstract selected policy, evidence, verifier, trust-boundary, and recovery-state semantics; they do not constitute a proof of complete Python-implementation equivalence, exhaustive real-world adversary coverage, cryptographic implementation security, operational spacecraft correctness, flightworthiness, or certification. The empirical campaign and its independently reproduced statistics remain the authority for the reported Study-2 effects, while the formal and implementation-bound assurance artifacts provide complementary evidence that key predeclared trust/recovery invariants were checked before those effects were generated.