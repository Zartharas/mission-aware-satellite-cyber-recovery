...
from .evidence import EvidenceClaim, EvidenceCondition, SignedEvidence, sign_claim
...
def apply_treatment(
    ...
    *,
    ...
    if condition is EvidenceCondition.STALE_OR_REPLAYED:
        ...
        if v2_mode == "stale":
            claim = replace(target.claim, issued_at_s=max(0.0, now_s - target.claim.valid_for_s - 1.0))
    ...
    if condition is EvidenceCondition.CONTRADICTORY:
        ...
        conflict_value = not target.claim.value if isinstance(target.claim.value, bool) else f"conflict:{target.claim.value}"
        conflict = replace(target.claim, source_id=alternate_source, value=conflict_value, sequence=target.claim.sequence + 1)
    ...
    if condition is EvidenceCondition.MANIPULATED:
        manipulated_value = not target.claim.value if isinstance(target.claim.value, bool) else f"tampered:{target.claim.value}"
        changed = list(evidence)
        changed[idx] = SignedEvidence(replace(target.claim, value=manipulated_value), target.signature_b64)
        return TreatmentResult(scenario, tuple(changed), (target_source,), "post_signature_value_tamper")
    ...
    if condition is EvidenceCondition.PARTIAL_COMPROMISE:
        ...
        signer = private_keys[target_source]
        false_value = not target.claim.value if isinstance(target.claim.value, bool) else f"false:{target.claim.value}"
        forged = replace(target.claim, value=false_value, sequence=target.claim.sequence + 1)
        changed = list(evidence)
        changed[idx] = sign_claim(forged, signer)
        return TreatmentResult(scenario, tuple(changed), (target_source,), "compromised_source_validly_signed_false_claim")
