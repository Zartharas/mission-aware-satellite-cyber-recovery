...
| WP4 | Testbed selection and architecture | In progress | VR13 was independently reviewed, published through PR #15, and merged at `5729f3218013...` with tree `ac1ff2d229b1...`. Production static-verifier execution #9 used verifier `3f56834da2aa...` exactly once, returned rc=4 with `SVF_SV_T063_RECEIPT_BEFORE_RUNTIME_ORDERING_VIOLATION`, produced no PASS evidence, consumed its authorization, and must not be rerun. Source forensics proved the command-aware production scanner accepts the exact candidate ordering, but the redundant legacy `_check_candidate_ordering()` line-index fallback misclassified `DOCKER_BIN="docker"` as an executed Docker operation before receipt validation. VR14 removes only that redundant fallback; corrected verifier `e803d59b0a0a...` passes focused T062/T063 controls and full selftest 78/0/0. Aggregate complete production verifier executions=9; corrected VR14 verifier production executions=0; candidate failure=false; static=PENDING. No new production verify, candidate execution, materialization, Docker/NOS3/Fortytwo, event injection, runtime, D-064, or WP5 authorization is granted. | Independently review and publish the exact VR14 source correction and execution-9 reconciliation before any new one-run static-verifier authorization decision; no runtime authorization |
