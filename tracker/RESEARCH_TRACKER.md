...
**WP5 — deterministic cyber-event library**

Evidence-first objective: each event must have an external/threat-model rationale, immutable ground truth, bounded policy-visible evidence, deterministic materialization, explicit expected/prohibited effects, and tests before any simulator adapter is written.
...
| WP5 | Deterministic event library | **In progress** | E1-E4 deterministic event adapters validated against the accepted NOS3 runtime; WP5 event library complete and WP6 response-policy implementation next |
