...
| WP4 | Testbed selection and architecture | In progress | VR9 one-run authorization was independently reviewed, published through PR #11, and merged at `ce82289c2dca...` with tree `727e5867a80a...`. Execution #7 used verifier `4a74b7e8e08c...` and candidate `599c534df37b...`, returned rc=4 with `SVF_SV_T067_CANDIDATE_LIVE_EXTERNAL_NOS3_MOUNT`, and is classified `INVALID_EXECUTION_VERIFIER_SOURCE_FALSE_POSITIVE_T067_TARGET_PATH_MISCLASSIFICATION_NO_STATIC_DISPOSITION`. Read-only classification proved 11 current T067 triggers, zero live `external/nos3` bind sources, and 11 transaction/workspace or runtime-config sources; the pre-correction T067 predicate matched `nos3` in the container target instead of requiring the bind source to be live NOS3. VR10 R0-R5 all stopped before repository mutation and are retained as harness-failure evidence; R3 exposed a dispatch-shape assumption, R4 exposed forbidden AST literal evaluation, and R5 exposed promotion of an observational runtime count into a gate. VR10 R6 removes those harness assumptions and corrects the read-only-confirmed T062-T069 scanner state defects (authorization/receipt state, Docker runtime/direct/prune detection, T067 bind-source classification, T069 writer direction, and the synthetic receipt fixture) and reconciles execution #7. Aggregate verifier executions are `7`; VR10-corrected verifier `46cf958d3bdd...` has `0` production executions; candidate failure remains false; static verification remains PENDING; no rerun/new production authorization; runtime false/0; D-064 BLOCKED; generator/candidate unchanged. | Independently review the exact five-file VR10 R6 T062-T069 verifier/governance correction, then publish it before any separate decision about a future production static-verifier authorization; no runtime authorization |
