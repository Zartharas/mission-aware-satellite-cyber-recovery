...
| WP4 | Testbed selection and architecture | In progress | VR16 is published on main with contract `0.4.13`, execution #10 consumed/no-rerun, v3 static verification=PASS, and candidate `599c534df37b...` accepted identity-only. D-064 preauthorization classification then established three blocking internal prototype gaps in the exact v3 runtime-material path: missing technical ACL validation, missing serialized transaction lock, and missing retained exclusive-writer evidence binding in the transaction receipt. Corrected remediation design R2 is now recorded locally as `WP4_D064_EXCLUSIVE_WRITER_TECHNICAL_CONTROL_REMEDIATION_DESIGN_R2_20260810.md` (SHA-256 `af2eefcc65b4...`) and lock `wp4-d064-exclusive-writer-technical-control-remediation-design-r2-lock.txt` (SHA-256 `9c4ed6ca97cf...`). The design preserves all v3 historical bytes and specifies a versioned successor transaction-v2 + generator/candidate-v4 + verifier-v4 chain, Darwin descriptor-bound no-extended-ACL policy, advisory `fcntl.flock` serialization, and SHA-256-bound retained host evidence. Successor contract revision remains UNASSIGNED; no successor source, execution #11, authorized root, D-064, runtime, or WP5 is authorized. | Independently review and publish the exact R2 remediation-design governance record; only after publication may a separate v4 source-implementation authorization be considered |
