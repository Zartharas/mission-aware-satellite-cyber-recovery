...
| WP4 | Testbed selection and architecture | In progress | VR11 one-run authorization was independently reviewed, published through PR #13, and consumed by exactly one production static-verifier execution #8 using verifier `46cf958d3bdd...` and candidate `599c534df37b...`. Execution #8 returned rc=4 / SV-T025 and was independently classified as an invalid verifier-source false positive: all authoritative proposed-candidate identities matched the candidate, but the separate production proposed-identity consistency helper used a synthetic-candidate fallback because run_verify did not bind the actual candidate body into its fixture. No candidate failure or static disposition was established; execution #8 must not be rerun. VR12 implements the minimal production binding correction, producing verifier `3f56834da2aa...`; static verification remains PENDING. No new production authorization is granted. Runtime/materialization/Docker/NOS3/Fortytwo/event injection remain unauthorized; D-064 remains BLOCKED; WP5 remains Not started. | Independently review and publish the exact VR12 five-file source/governance correction before any future production static-verifier authorization decision |
