...
| WP4 | Testbed selection and architecture | In progress | VR8 T068 verifier-source correction was independently reviewed, published through PR #10, and merged on clean main at `889672b889d1...` with accepted tree `e60339060548...`. Execution #6 remains consumed and invalid with no candidate static disposition. The corrected verifier is `4a74b7e8e08c...`; corrected-verifier production executions remain `0`; aggregate complete production verifier executions remain `6`; candidate failure remains false; static verification remains PENDING. VR9 records governance authorization for exactly one future corrected-verifier production `--verify` attempt only; the authorization is not consumed in this gate. Runtime, materialization, Docker/NOS3/Fortytwo, event injection, WP5 execution, and D-064 remain blocked; generator/candidate bytes remain unchanged. | Independently review and publish the exact four-file VR9 post-publication one-run static-verifier authorization before executing the single authorized production `--verify`; no runtime authorization |
