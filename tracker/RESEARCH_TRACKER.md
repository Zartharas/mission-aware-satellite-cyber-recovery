...
- [ ] Implement the versioned bounded runtime candidate (`prepare_passive_time_witness_runtime_candidate_v2.sh`) and cleanup controls under D-062 without authorizing runtime (the exact observation-duration value is unresolved and must be frozen during D-062; D-063 is a separate static gate and D-064 is the sole authorization disposition)
...
D-060 (2026-07-28) governance-accepted the passive time-witness static gate and locked the implementation and runtime-candidate hashes while retaining a closed runtime-authorization gate. D-061 (2026-07-28) accepted the runtime-control remediation design and froze the phase separation: D-062 implements the versioned v2 candidate and generator (runtime unauthorized), D-063 executes and reviews a separate fail-closed static gate for v2 (runtime unauthorized), and D-064 is the sole disposition that may consider authorizing exactly one bounded passive telemetry attempt. The following remain blocked pending that separate D-064 authorization:
