...
The Zenodo v1.0.0 record is the **Study-1 evidence-of-record** and must not be described as containing Study-2 source observations.
...
The two populations must remain separate throughout manuscript text, displays, code/data availability, and submission metadata.
...
No `WP12` or `Phase 8` is created by this tracker. New downstream work is described functionally unless a new protocol/work package is explicitly frozen.
...
- controlled software-in-the-loop research only;
...
- no post-hoc seed replacement, outcome-dependent exclusion, or new campaign execution to improve journal results;
