...
SEED=9301
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-wp8-observability-binding-dev}"
...
EVIDENCE="$ROOT/results/wp8/runtime-binding/observability/$RUN_ID"
...
RUNNER_SHA="$(shasum -a 256 "$ROOT/scripts/run_wp8_observability_binding_preflight.sh" | awk '{print $1}')"

PHASE="FACTOR_EVENT_MATERIALIZATION"

PYTHONPATH="$ROOT" python3 - \
  "$FACTOR_JSON" "$EVENT_JSON" "$RUN_ID" "$SEED" "$REPO_COMMIT" <<'PY'
import json
import sys
from pathlib import Path
...
factor_path, event_path, run_id, seed, repo_commit = sys.argv[1:]
seed = int(seed)
