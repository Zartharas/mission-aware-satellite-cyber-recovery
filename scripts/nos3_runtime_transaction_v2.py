...
def _build_argparser():
    p = argparse.ArgumentParser(
        description="NOS3 runtime-material v4 transaction authorization CLI "
                    "(Checkpoint 2PB2B-A-R2, host-only, fail-closed).")
    p.add_argument("--repo-root", default=os.getcwd())
    g = p.add_mutually_exclusive_group(required=False)
    g.add_argument("--materialize-v4-transaction", action="store_true")
...
def main(argv=None):
    ...
    if args.materialize_v4_transaction:
        missing = [a for a in ("contract", "manifest", "candidate",
                               "authorized_root", "final_basename")
                   if not getattr(args, a, None)]
        if missing:
            print("V4_TRANSACTION_AUTHORIZATION=CLOSED")
    ...
    try:
        ...
        def _current_contract_static_gate_closed(result):
            ...
            return (
                ...
                and detail == "v4 contract schema not exact int 1"
