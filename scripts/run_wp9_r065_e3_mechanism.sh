...
OUTPUT_JSON="$4"
cd "$ROOT"
for command in git python3 docker shasum; do
...
test -z "$(git status --short)" || {
  echo "[ERROR] repository worktree must be clean before R-065 E3 runtime" >&2
  exit 1
}
...
f = r["factor_context"]
print(
    r["case_id"], r["cell_id"], r["development_seed"], r["repo_commit"],
    r["run_id"], r["actual_effective_policy_id"], r["selected_action"],
    f["contact_condition_id"], f["evidence_condition_id"],
    r["runtime_variant"], r["evidence_directory"],
)
PY
)
EOF_REQUEST
case "$CASE_ID" in
  ...
  *) echo "[BLOCKED] E3 harness supports Z04-Z09 only" >&2; exit 3 ;;
esac
[[ "${WP9_R065_AUTHORIZED_CASE:-}" == "$CASE_ID" ]] || {
  echo "[BLOCKED] R-065 authorization case mismatch" >&2
  exit 3
}
[[ "${WP9_R065_AUTHORIZED_SEED:-}" == "$SEED" ]] || {
  echo "[BLOCKED] R-065 authorization seed mismatch" >&2
  exit 3
}
[[ "${WP9_R065_AUTHORIZED_REPO_SHA:-}" == "$REPO_COMMIT" ]] || {
  echo "[BLOCKED] R-065 authorization SHA mismatch" >&2
  exit 3
}
[[ "$REQUEST_SHA" == "$REPO_COMMIT" ]] || {
  echo "[BLOCKED] R-065 request SHA mismatch" >&2
  exit 3
}

EXPECTED_EVIDENCE="results/wp9/development/r065/integration/$RUN_ID"
[[ "$REQUEST_EVIDENCE" == "$EXPECTED_EVIDENCE" ]] || {
  echo "[ERROR] E3 evidence directory escaped R-065 development namespace" >&2
  exit 1
}
...
EXPECTED_OUTPUT="$OBS/${CASE_SAFE}-driver-result.json"
[[ "$(cd "$(dirname "$REQUEST_JSON")" && pwd)/$(basename "$REQUEST_JSON")" == "$EXPECTED_REQUEST" ]] || {
  echo "[ERROR] E3 request path is not retained evidence path" >&2
  exit 1
}
[[ "$(cd "$(dirname "$OUTPUT_JSON")" && pwd)/$(basename "$OUTPUT_JSON")" == "$EXPECTED_OUTPUT" ]] || {
  echo "[ERROR] E3 output path is not retained evidence path" >&2
  exit 1
}
...
GATEWAY="mascr-$SAFE_ID-r065-e3-gateway"
GATEWAY_ALIAS="r065-e3-gateway"
...
emit_invalid() {
  ...
  python3 - "$INVALID_JSON" "$RUN_ID" "$CASE_ID" "$CELL_ID" "$SEED" "$PHASE" "$rc" "$REPO_COMMIT" "$DEVELOPMENT_SEED_CONSUMED" <<'PY'
import json
import sys
from pathlib import Path
p, run_id, case_id, cell, seed, phase, rc, commit, consumed = sys.argv[1:]
Path(p).write_text(json.dumps({
    "schema": 1,
    "decision_id": "R-065",
    "classification": "WP9_R065_E3_BOUNDED_INTEGRATION_RUN_INVALID",
    "run_id": run_id,
    "case_id": case_id,
    "cell_id": cell,
...
cleanup() {
  ...
  if [[ "$RESULT" == "PASS" && "$rc" -eq 0 ]]; then
    echo "WP9_R065_E3_MECHANISM_RUNTIME=PASS"
    ...
    echo "WP9_R065_E3_MECHANISM_RUNTIME=FAIL" >&2
...
echo "r065_e3_runtime_authorization=PASS"
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$ROOT" python3 - \
  "$REQUEST_JSON" "$EVENT_JSON" "$APPROVED" "$TAMPERED" "$MANIFEST" "$VERIFY_TAMPERED" <<'PY'
import json
import sys
from pathlib import Path
from src.mission_recovery.update_artifacts import (
    build_approved_update, build_tampered_update, build_manifest,
    sha256_hex, verify_candidate,
)
req, event_path, approved_path, tampered_path, manifest_path, verify_path = sys.argv[1:]
r = json.loads(Path(req).read_text(encoding="utf-8"))
approved = build_approved_update()
tampered = build_tampered_update()
manifest = build_manifest()
verify = verify_candidate(tampered, manifest)
assert sha256_hex(approved) == "42945a2622fa351b3a3fdc31e002cbe326cb7a42a958ee757f317abea67b6697"
assert sha256_hex(tampered) == "ff96d61205cc2c49b6d7d73fc36b9544c0deea79d7a9304cc1fb9f1f8986053d"
...
Path(verify_path).write_text(json.dumps(verify, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
...
(
  ...
  docker exec "$CFS" sh -lc \
    'slot="$1"; expected="$2"; echo R065_EVENT_SLOT_WATCHER_READY; i=0; while [ "$i" -lt 3000 ]; do if [ -f "$slot" ]; then observed="$(sha256sum "$slot" 2>/dev/null | awk "{print \$1}")"; [ "$observed" = "$expected" ] && { echo "R065_EVENT_SLOT_SHA=$observed"; exit 0; }; [ -n "$observed" ] && { echo "R065_EVENT_SLOT_UNEXPECTED_SHA=$observed" >&2; exit 2; }; fi; i=$((i+1)); sleep 0.01; done; exit 1' \
    sh "$STAGE_BACKING" "$TAMPERED_SHA" >"$EVENT_WATCH_LOG" 2>&1
  watcher_rc=$?
  if [[ "$watcher_rc" -eq 0 ]]; then
    observed="$(awk -F= '/^R065_EVENT_SLOT_SHA=/{print $2;exit}' "$EVENT_WATCH_LOG")"
    [[ "$observed" == "$TAMPERED_SHA" ]] || exit 3
    mono_ns > "$EVENT_SUCCESS_NS_FILE"
    printf '%s\n' "$observed" > "$EVENT_SLOT_SHA_FILE"
  fi
  exit "$watcher_rc"
) &
EVENT_WATCH_PID=$!
...
for _ in $(seq 1 200); do
  grep -Fq R065_EVENT_SLOT_WATCHER_READY "$EVENT_WATCH_LOG" 2>/dev/null && {
    READY=1
    break
  }
  kill -0 "$EVENT_WATCH_PID" >/dev/null 2>&1 || break
  sleep 0.01
done
[[ "$READY" -eq 1 ]]
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$ROOT" python3 - "$REQUEST_JSON" "$POLICY_JSON" <<'PY'
import json
import sys
from pathlib import Path
...
r = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
f = r["factor_context"]
d = evaluate_wp9_policy(f["policy_id"], r["event_instance"])
assert d["delegated_policy_id"] == r["actual_effective_policy_id"]
assert d["selected_action"] == r["selected_action"]
assert d["oracle_ground_truth_read"] is False
Path(sys.argv[2]).write_text(json.dumps(d, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
...
[[ "$(cat "$EVENT_SLOT_SHA_FILE")" == "$TAMPERED_SHA" ]]
...
if [[ "$SELECTED_ACTION" == "RESTRICT_HIGH_RISK_COMMANDS" ]]; then
  ...
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$ROOT" python3 -m \
    ...
    --output-json "$ROLLBACK_JSON" >/dev/null
  POLICY_ENFORCEMENT_NS="$(mono_ns)"
  RESPONSE_BOUNDARY_NS="$POLICY_ENFORCEMENT_NS"
...
print(str(r["rollback_request_validated"]).lower(), str(r["replacement_source_verified"]).lower())
PY
)
EOF_R
  [[ "$ROLLBACK_VALIDATED" == "true" && "$SOURCE_VERIFIED" == "true" ]]
...
import sys
from pathlib import Path
p, contact, available, missed, observed_ns = sys.argv[1:]
Path(p).write_text(json.dumps({
    "schema": 1,
    "decision_id": "R-065",
...
PY
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$ROOT" python3 - "$REQUEST_JSON" "$HANDOFF_JSON" <<'PY'
import json
import sys
from pathlib import Path
from src.mission_recovery.wp9_static_contracts import evaluate_wp9_policy
r = json.loads(Path(sys.argv[1]).read_text())
d = evaluate_wp9_policy("P5", r["event_instance"])
assert d["delegated_policy_id"] == "P5"
assert d["selected_action"] == "REQUEST_VERIFIED_ROLLBACK"
assert d["oracle_ground_truth_read"] is False
Path(sys.argv[2]).write_text(json.dumps(d, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
  HANDOFF_NS="$(mono_ns)"
...
EOF_R
  ...
  echo "[ERROR] unsupported R-065 E3 action: $SELECTED_ACTION" >&2
  exit 1
fi
...
if [[ "$SELECTED_ACTION" == "RESTRICT_HIGH_RISK_COMMANDS" ]]; then
  ...
  send_gateway_command modeled_attacker sample_reset_counters attacker-reset-probe-1.json
  send_gateway_command modeled_attacker sample_reset_counters attacker-reset-probe-2.json
  send_gateway_command authorized_ground sample_noop authorized-noop-probe.json
  ...
  read -r GATEWAY_DECISION_COUNT ATTACKER_FORWARDED_COUNT AUTHORIZED_GATEWAY_FORWARDED <<EOF_G
$(python3 - "$GATEWAY_DECISIONS" <<'PY'
import json
import sys
from pathlib import Path
rows = [json.loads(x) for x in Path(sys.argv[1]).read_text().splitlines() if x.strip()]
auth = [x for x in rows if x.get("source_id") == "authorized_ground"]
print(
    len(rows),
    sum(1 for x in rows if x.get("source_id") == "modeled_attacker" and x.get("forwarded") is True),
    str(len(auth) == 1 and auth[0].get("forwarded") is True).lower(),
)
PY
)
EOF_G
...
import sys
from pathlib import Path
p, sha, observed_ns = sys.argv[1:]
Path(p).write_text(json.dumps({"sha256": sha, "observed_ns": int(observed_ns)}, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
...
import sys
from pathlib import Path
out, slot, rollback, noop, health, manifest, terminal, delta, observed_ns = sys.argv[1:]
delta = int(delta)
refs = {
    "approved_version": slot,
    "integrity_measurement_valid": terminal,
    "authorization_valid": rollback,
    "measured_state_current": slot,
    "authorized_command_path_restored": noop,
    "ground_spacecraft_state_agreed": slot,
    "required_telemetry_restored": health,
    "health_checks_passed": health,
    "no_residual_unauthorized_state": slot,
    "recovery_manifest_complete": manifest,
}
satisfied = {key: True for key in refs}
satisfied["authorized_command_path_restored"] = delta == 1
rows = {
    key: {
        "available_current": True,
        "criterion_satisfied": bool(satisfied[key]),
        "evidence_ref": str(Path(ref)),
        "observed_ns": int(observed_ns),
    }
    for key, ref in refs.items()
}
Path(out).write_text(json.dumps(rows, sort_keys=True, indent=2) + "\n", encoding="utf-8")
PY
...
print(str(all(x["available_current"] and x["criterion_satisfied"] for x in rows.values())).lower())
PY
)"
...
python3 - \
  "$MEASUREMENT_JSON" "$CRITERIA_JSON" "$RUN_ID" "$RUN_START_UTC" \
  "$RUN_START_NS" "$EVENT_ACTIVATION_NS" "$EVENT_SUCCESS_NS" \
  "$POLICY_SELECTION_NS" "$POLICY_ENFORCEMENT_NS" "$RESPONSE_BOUNDARY_NS" \
  "$OBSERVATION_COMPLETE_NS" "$POST_SLOT_SHA" "$RUNTIME_HEALTH_PASSED" \
  "$AUTHORIZED_NOOP_DELTA" "$EFFECTIVE_POLICY" "$MATCHED_ATTACKER_PROBES" \
  ...
  "$AUTH_AVAILABLE_AT_BOUNDARY" "$MISSED_CONTACT_WINDOWS" <<'PY'
import json
import sys
from pathlib import Path
(
    path, criteria_path, run_id, run_start_utc, run_start_ns,
    activation_ns, success_ns, selection_ns, enforcement_ns,
    response_boundary_ns, complete_ns, slot_sha, runtime_health,
    noop_delta, effective, attackers, decision_count, attacker_forwarded,
    authorized_forwarded, attacker_delta, rollback_validated,
    source_verified, temp_absent, rollback_ns, trusted, trusted_ns,
    ground_waited, authorization_ns, handoff_ns, available, missed,
) = sys.argv[1:]
as_bool = lambda value: value.lower() == "true"
row = {
    "schema": 1,
    "run_id": run_id,
    "run_start_utc": run_start_utc,
    "run_start_ns": int(run_start_ns),
    "event_activation_ns": int(activation_ns),
    "event_success_observed_ns": int(success_ns),
    "policy_selection_ns": int(selection_ns),
    "policy_enforcement_ns": int(enforcement_ns),
    "response_boundary_ns": int(response_boundary_ns),
    "observation_complete_ns": int(complete_ns),
    "event_activation_observed": True,
    "event_slot_sha256": "ff96d61205cc2c49b6d7d73fc36b9544c0deea79d7a9304cc1fb9f1f8986053d",
    "runtime_health_passed": as_bool(runtime_health),
    "policy_selection_not_gated_on_event_success": True,
    "authorized_noop_attempted": 1,
    "authorized_noop_marker_delta": int(noop_delta),
    "post_response_slot_sha256": slot_sha,
    "ground_authorization_waited": as_bool(ground_waited),
}
if effective == "P2":
    row.update({
        "update_containment_observed": False,
        "matched_attacker_probe_count": int(attackers),
        "gateway_decision_count": int(decision_count),
        "attacker_gateway_forwarded_count": int(attacker_forwarded),
        "authorized_noop_gateway_forwarded": as_bool(authorized_forwarded),
        "gateway_action": "RESTRICT_HIGH_RISK_COMMANDS",
        "observed_post_enforcement_attacker_reset_marker_delta": int(attacker_delta),
    })
else:
    row.update({
        "rollback_request_validated": as_bool(rollback_validated),
        "approved_replacement_source_verified": as_bool(source_verified),
        "temporary_recovery_state_absent": as_bool(temp_absent),
        "rollback_complete_ns": int(rollback_ns),
        "trusted_recovery_confirmed": as_bool(trusted),
        "recovery_criteria": json.loads(Path(criteria_path).read_text()),
    })
    if as_bool(trusted):
    ...
    if effective == "P6":
        row.update({
            ...
            "authorization_observed_ns": int(authorization_ns),
            "handoff_ns": int(handoff_ns),
            "authorization_available_at_response_boundary": as_bool(available),
...
PHASE="CLEANUP_AUDIT"
docker rm -f "$GATEWAY" "$CFS" >/dev/null 2>&1 || true
docker network rm "$NETWORK" >/dev/null 2>&1 || true
if docker ps -a --format '{{.Names}}' | grep -Fq "$SAFE_ID"; then
  echo "[ERROR] residual R-065 E3 container remains" >&2
  exit 1
fi
if docker network inspect "$NETWORK" >/dev/null 2>&1; then
  echo "[ERROR] residual R-065 E3 network remains" >&2
  exit 1
fi
