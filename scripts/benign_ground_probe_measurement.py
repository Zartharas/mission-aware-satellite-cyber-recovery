#!/usr/bin/env python3
"""Deterministic measurement-only ground probe for the WP4 benign baseline.

This program has no event-injection or telemetry-setup capability. It binds the
internal telemetry endpoint, establishes a stable SAMPLE housekeeping baseline,
transmits exactly one frozen SAMPLE_NOOP_CC packet, and records separated
immutable-ground and policy-visible evidence.
"""
...
class GroundProbe:
    ...
    def finalize(self, classification: str, reason: str | None = None) -> tuple[str, str]:
        ...
        result: dict[str, Any] = {
            ...
            "telemetry_activation": {
                "mechanism": "SC_RTS001_TO_LAB_OUTPUT_ENABLE",
                "ground_setup_transmissions": 0,
                "destination_alias": "active-gs",
    ...
    def run(self) -> int:
        ...
        try:
            ...
            print(
                f"GROUND_PROBE_INVALID reason=telemetry_bind_failed error={exc}",
                file=sys.stderr,
                flush=True,
            )
            return 3
...
def self_test() -> None:
    ...
    print("BENIGN_GROUND_PROBE_MEASUREMENT_SELF_TEST=PASS")
