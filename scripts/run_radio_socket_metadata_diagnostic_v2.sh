...
radio_anchor = (
    f"      --network-alias generic-radio-sim {slash}\\n"
    f"      --env TCP_GROUND=0 --env MULTI_GDS=0 {slash}\\n"
)
radio_instrumented = (
    f"      --network-alias generic-radio-sim {slash}\\n"
    f"      --env TCP_GROUND=0 --env MULTI_GDS=0 {slash}\\n"
    f"      --env LD_PRELOAD=/tmp/libradio_socket_metadata_shim.so {slash}\\n"
    f"      --env RADIO_SOCKET_TRACE_PATH=/evidence-socket-metadata/radio-socket-metadata.log {slash}\\n"
    f"      --mount \"type=bind,source=$SHIM_SO,target=/tmp/libradio_socket_metadata_shim.so,readonly\" {slash}\\n"
    f"      --mount \"type=bind,source=$SOCKET_METADATA_DIR,target=/evidence-socket-metadata\" {slash}\\n"
)
add(
