...
POLICY_ID="${POLICY_ID:?POLICY_ID must be P0 or P4}"
case "$POLICY_ID" in
  P0|P4) ;;
  *) echo "[ERROR] unsupported POLICY_ID=$POLICY_ID" >&2; exit 1 ;;
esac

RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-wp6-$POLICY_ID-p4}"
...
EVIDENCE="$ROOT/results/wp6/p4/$RUN_ID"
...
ATTACK_SEND="$GROUND/attacker-noop-send.json"
AUTH_SEND="$GROUND/authorized-noop-send.json"
...
cleanup() {
  ...
  if [[ "$RESULT" == PASS && "$rc" -eq 0 ]]; then
    echo "WP6_P4_POLICY_EFFECT_TRIAL=PASS"
    echo "policy_id=$POLICY_ID"
    echo "evidence_directory=$EVIDENCE"
  else
    echo "WP6_P4_POLICY_EFFECT_TRIAL=FAIL" >&2
...
case "$POLICY_ID:$ACTION" in
  P0:OBSERVE_ONLY|P4:ENTER_SAFE_MODE) ;;
  *)
...
send_noop modeled_attacker "$(basename "$ATTACK_SEND")"
...
if [[ "$POLICY_ID" == P0 ]]; then
  ...
  test "$AFTER_ATTACK" -eq "$BASELINE" || {
    echo "[ERROR] P4 attacker routine command reached cFS" >&2
    exit 1
  }
  echo "attacker_noop_acceptance_delta=0"
fi

send_noop authorized_ground "$(basename "$AUTH_SEND")"

if [[ "$POLICY_ID" == P0 ]]; then
  AFTER_AUTH="$(wait_marker_exact "$AFTER_ATTACK" 1 authorized)"
  echo "authorized_noop_acceptance_delta=1"
else
  sleep 3
  AFTER_AUTH="$(count_noop_marker)"
  test "$AFTER_AUTH" -eq "$AFTER_ATTACK" || {
    echo "[ERROR] P4 authorized routine command reached cFS" >&2
    exit 1
  }
  echo "authorized_noop_acceptance_delta=0"
fi
...
expected=[True,True] if policy=="P0" else [False,False]
actual=[r["forwarded"] for r in decisions]
assert actual==expected,(actual,expected)

print("gateway_routine_command_pair=PASS")
print("gateway_safe_mode_decisions=PASS")
...
python3 - \
  ...
  "$BASELINE" "$AFTER_ATTACK" "$AFTER_AUTH" \
...
after_auth=int(after_auth)

attacker_delta=after_attack-baseline
authorized_delta=after_auth-after_attack

expected=1 if policy_id=="P0" else 0
assert attacker_delta==expected
assert authorized_delta==expected

summary={
    "schema":1,
    "classification":"WP6_P4_POLICY_EFFECT_TRIAL_PASS",
    "policy_id":policy_id,
    ...
    "legitimate_command_rejection_rate":0.0 if policy_id=="P0" else 1.0,
    "authorized_routine_command_path_preserved":policy_id=="P0",
    "modeled_safe_mode_command_gate_active":policy_id=="P4",
    "gateway_ingress_sha256":ingress_sha,
    "gateway_decisions_sha256":decisions_sha,
    "validated_nominal_runtime_pass":True,
    "native_cfs_safe_mode_claim":False,
    "spacecraft_hardware_mode_claim":False,
    "mission_objective_completion_claim":False,
    "final_mission_cost_claim":False,
...
if [[ "$POLICY_ID" == P0 ]]; then
  echo "modeled_unauthorized_effect_completed=true"
  echo "legitimate_command_rejection_rate=0.0"
  echo "authorized_routine_command_path_preserved=true"
  echo "modeled_safe_mode_command_gate_active=false"
else
  echo "modeled_unauthorized_effect_completed=false"
  echo "legitimate_command_rejection_rate=1.0"
  echo "authorized_routine_command_path_preserved=false"
  echo "modeled_safe_mode_command_gate_active=true"
fi

echo "validated_nominal_runtime_pass=true"
echo "native_cfs_safe_mode_claim=false"
echo "mission_objective_completion_claim=false"
echo "final_mission_cost_claim=false"
