...
WRAPPER="$ROOT/scripts/run_radio_socket_metadata_diagnostic_v3.sh"
...
assert contract["gate"]["radio_socket_metadata_runtime_wrapper_static_verification"] == "SUPERSEDED_BY_PRE_RUNTIME_ASSERTION_FAILURE"
assert contract["gate"]["radio_socket_metadata_runtime_wrapper_v3_static_verification"] == "PENDING"
...
assert contract["runtime_wrapper_requirements"]["wrapper_gate_assertion_must_be_mode_aware"] is True
assert contract["pre_runtime_attempt_results"][-1]["runtime_authorization_consumed"] is True
assert contract["pre_runtime_attempt_results"][-1]["docker_invoked"] is False
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/radio-metadata-wrapper-v3.XXXXXX")"
chmod 755 "$TMP"
...
[[ -s "$TMP/libradio_socket_metadata_shim.so" ]] || {
  echo "[ERROR] Network-disabled v3 verification did not produce the shim shared object." >&2
  exit 1
}
actual_shim_so_sha="$(shasum -a 256 "$TMP/libradio_socket_metadata_shim.so" | awk '{print $1}')"
[[ "$actual_shim_so_sha" == "$expected_shim_so_sha" ]] || {
  echo "[ERROR] V3 wrapper shim build differs from the accepted component gate." >&2
  exit 1
}

EMITTED="$TMP/generated-radio-socket-metadata-runtime-v3.sh"
RADIO_SOCKET_METADATA_WRAPPER_VERIFY_ONLY=1 \
RADIO_SOCKET_METADATA_EMIT_PATH="$EMITTED" \
bash "$WRAPPER"

[[ -s "$EMITTED" ]] || {
  echo "[ERROR] V3 metadata wrapper did not emit the generated runtime." >&2
  exit 1
}
bash -n "$EMITTED"
...
for forbidden in (
    ...
    if forbidden in text:
        raise SystemExit(f"forbidden generated v3 runtime token present: {forbidden}")

docker_socket_mount_markers = (
    '--mount', '--volume', 'source=', 'target=',
    '-v /var/run/docker.sock', '-v=/var/run/docker.sock',
)
for line in text.splitlines():
    if '/var/run/docker.sock' in line and any(marker in line for marker in docker_socket_mount_markers):
        raise SystemExit(f"forbidden generated Docker socket mount present: {line.strip()}")
...
[[ "$after_containers" == 0 && "$after_networks" == 0 ]] || {
  echo "[ERROR] V3 wrapper static verification left project-labeled Docker resources." >&2
  exit 1
}

echo "runtime_wrapper_v3_sha256=$(shasum -a 256 "$WRAPPER" | awk '{print $1}')"
echo "generated_runtime_v3_sha256=$(shasum -a 256 "$EMITTED" | awk '{print $1}')"
...
echo "RADIO_SOCKET_METADATA_RUNTIME_WRAPPER_V3_STATIC_VERIFICATION_STATUS=PASS"
